#' @export
run_mas_segment <- function(maindir = maindir,
                            subdir = "MAS",
                            om_sim_num = NULL,
                            casedir = casedir,
  ...
  if (!("r4MAS" %in% installed.packages()[, "Package"]))
    stop("Please install r4MAS!")
  
  setwd(file.path(casedir, "output", subdir))
  unlink(list.files(file.path(casedir, "output", "MAS"), full.names = TRUE), recursive = TRUE)
  sapply(1:om_sim_num, function(x)
    dir.create(file.path(
      casedir, "output", subdir, paste("s", x, sep = "")
    )))
  ...
  for (om_sim in begin[mpi.comm.rank()]:end[mpi.comm.rank()]) {
    ...
    if (om_input$sel_fleet$fleet1$pattern == 1) {
      ...
      fleet_selectivity$a50$value <-
        om_input$sel_fleet$fleet1$A50.sel
      ...
      fleet_selectivity$slope$value <-
        1 / om_input$sel_fleet$fleet1$slope.sel
    ...
    if (om_input$sel_fleet$fleet1$pattern == 2) {
      ...
      fleet_selectivity$alpha_asc$value <-
      ...
      fleet_selectivity$beta_asc$value <-
      ...
      fleet_selectivity$alpha_desc$value <-
        om_input$sel_fleet$fleet1$A50.sel2
      ...
      fleet_selectivity$beta_desc$value <-
        om_input$sel_fleet$fleet1$slope.sel2
    ...
    for (i in 1:om_input$survey_num) {
      ...
      # survey_comp[[i]]$id <- i
      survey_comp[[i]]$values <-
        as.vector(t(em_input$survey.age.obs[[i]]))
      survey_comp[[i]]$sample_size <-
        rep(em_input$n.survey[[i]], times = om_input$nyr)
    }
    
    # Likelihood component settings
    
    survey_index_comp_nll <-
      vector(mode = "list", length = om_input$survey_num)
    survey_age_comp_nll <-
      vector(mode = "list", length = om_input$survey_num)
    for (i in 1:om_input$survey_num) {
    ...
    for (i in 1:om_input$survey_num) {
      ...
      if (selectivity_option == 1) {
        ...
        survey_selectivity[[i]]$a50$value <-
          om_input$sel_survey[[i]]$A50.sel
        ...
        survey_selectivity[[i]]$slope$value <-
          1 / om_input$sel_survey[[i]]$slope.sel
      ...
      if (selectivity_option == 2) {
        ...
        survey_selectivity[[i]]$alpha_asc$value <-
          om_input$sel_survey[[i]]$A50.sel1
        ...
        survey_selectivity[[i]]$beta_asc$value <-
          om_input$sel_survey[[i]]$slope.sel1
        ...
        survey_selectivity[[i]]$alpha_desc$value <-
          om_input$sel_survey[[i]]$A50.sel2
        ...
        survey_selectivity[[i]]$beta_desc$value <-
          om_input$sel_survey[[i]]$slope.sel2
...
#' @export
run_mas_mpi <- function(maindir = maindir,
                    subdir = "MAS",
                    om_sim_num = NULL,
                    casedir = casedir,
                    em_bias_cor = em_bias_cor,
                    input_filename = NULL) {
  print("Running MAS in MPI mode.....now")
